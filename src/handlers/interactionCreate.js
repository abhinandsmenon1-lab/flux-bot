  ...
  async function handleButton(interaction) {
    ...
    if (id.startsWith('lb_prev_') || id.startsWith('lb_next_')) {
      ...
      if (interaction.user.id !== ownerId) {
        return interaction.reply({ content: 'Only the person who opened this leaderboard can change its page.', ephemeral: true });
      }

      const leaderboardCommand = client.commands.get('leaderboard');
      if (!leaderboardCommand) return interaction.reply({ content: 'Leaderboard command is unavailable.', ephemeral: true });

      const nextPage = Math.max(0, currentPage + (direction === 'next' ? 1 : -1));
      const config = await getConfig(interaction.guild.id);
      const { rows, totalPages } = await leaderboardCommand.getPage(interaction.guild.id, nextPage);
      const safePage = Math.min(nextPage, totalPages - 1);
      const pageData = safePage === nextPage ? { rows, totalPages } : await leaderboardCommand.getPage(interaction.guild.id, safePage);
      const embed = leaderboardCommand.buildLeaderboardEmbed(config, pageData.rows, safePage, pageData.totalPages);
      return interaction.update({
        embeds: [embed],
        components: pageData.totalPages > 1 ? [leaderboardCommand.buildButtons(safePage, pageData.totalPages, ownerId)] : []
      });
    }
