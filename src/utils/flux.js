...
async function getLeaderboard(guildId, limit = 10, skip = 0) {
  const rows = await FluxBalance.find({ guildId, balance: { $gt: 0 } })
    .sort({ balance: -1, userId: 1 })
    .skip(skip)
    .limit(limit);
...
async function getLeaderboardCount(guildId) {
  return FluxBalance.countDocuments({ guildId, balance: { $gt: 0 } });
}

module.exports = { getBalance, setBalance, addBalance, tryDeduct, increment, getLeaderboard, getLeaderboardCount };
